 ...
 * 
 * */    
    deepSaveObject(key,data,keyFunction =this.options.keyFunction, callback)
    {
        try 
        {
 ...
 * 
 */    
    getAllObjects(key , callback) {
	try {
        key = this.options.createKey(key);
        this.client.multi().smembers(key,(error, replies)=>{
            _.forEach(replies,(reply)=>
            {
                this.client.hgetall('',redis.print);
            });
        }).exec( (error,newData)=>{
            let data = [];
             _.forEach(newData,(reply)=>
            {
                    console.log(newData);
               data.push (this.options.formatObjectValue(reply));
            });
 ...
 * 
 */    
    deleteAll(key, callback) {
      try{
        key = this.options.createKey(key); 
        let multi = this.client.multi();
        this.client.smembers(key,(error, replies)=>{
            multi.del(key);
            _.forEach(replies,(reply)=>
            {
                multi.del(reply);
